...
%Create a column called Subject which contains subject ids extracted from
%the row names.  
All.Subject = All.Properties.RowNames;

%Rename all to CT. 
CT=All;

%Create 5 small tables which are subsetted from the table CT.  The fist
%table contains all the behavioral outliers, the second table contains all
%the rotation outliers for drug sessions, the third contains the
%translation errors for drug sessions, the fourth is rotation outliers for
%placebo sessions and the fifth is translation outliers for placebo
%sessions.  
TBehav = CT(:, {'Aud_Miss', 'Aud_Inc', 'Mot_Miss', 'Vis_Miss', 'Vis_Inc', 'Subject'});
TMotRD = CT(:, {'R_DAUD', 'R_DCON', 'R_DMOT', 'R_DTAC', 'R_DVIS', 'Subject'});
TMotTD = CT(:, {'T_DAUD', 'T_DCON', 'T_DMOT', 'T_DTAC', 'T_DVIS', 'Subject'});
TMotRP = CT(:, {'R_PAUD', 'R_PCON', 'R_PMOT', 'R_PTAC', 'R_PVIS', 'Subject'});
TMotTP = CT(:, {'T_PAUD', 'T_PCON', 'T_PMOT', 'T_PTAC', 'T_PVIS', 'Subject'});

%Create a new column "Sum" which sums all the values for the behavioral
%outliers. 
TBehav.Sum = (TBehav.Aud_Miss + TBehav.Aud_Inc + TBehav.Mot_Miss + TBehav.Vis_Miss + TBehav.Vis_Inc);

%Create a cariable "toDelete".  Since you cannot say "Delte if T.Behav.Sum
...
toDelete = TBehav.Sum < 0.0000000000001;
...
%Change teh format of the subject column to be character. 
TBehav.Subject = char(TBehav.Subject);
...
TBehav.ID = TBehav.Subject(:,6:8);

%Create a new table, TBehav_Final which is TBehav but sorted by ID, that
%is, in numerical order. 
TBehav_Final = sortrows(TBehav,'ID');

%Delte the ID column. 
TBehav_Final.ID = [];

%Delte the subject column.  (Row Names still has subject ID). 
TBehav_Final.Subject =[];


% Repeat what was done above for the other 4 small tables we made. 
TMotRD.Sum = (TMotRD.R_DAUD + TMotRD.R_DCON + TMotRD.R_DMOT + TMotRD.R_DTAC + TMotRD.R_DVIS);
toDelete = TMotRD.Sum < 0.0000000000001;
TMotRD(toDelete, :) = [];
TMotRD.Sum = [];
TMotRD.Subject = char(TMotRD.Subject);
TMotRD.ID = TMotRD.Subject(:,6:8);
TMotRD_Final = sortrows(TMotRD,'ID');
TMotRD_Final.ID = [];
TMotRD_Final.Subject =[];

TMotTD.Sum = (TMotTD.T_DAUD + TMotTD.T_DCON + TMotTD.T_DMOT + TMotTD.T_DTAC + TMotTD.T_DVIS);
toDelete = TMotTD.Sum < 0.0000000000001;
TMotTD(toDelete, :) = [];
TMotTD.Sum = [];
TMotTD.Subject = char(TMotTD.Subject);
TMotTD.ID = TMotTD.Subject(:,6:8);
TMotTD_Final = sortrows(TMotTD,'ID');
TMotTD_Final.ID = [];
TMotTD_Final.Subject =[];

TMotRP.Sum = (TMotRP.R_PAUD + TMotRP.R_PCON + TMotRP.R_PMOT + TMotRP.R_PTAC + TMotRP.R_PVIS);
toDelete = TMotRP.Sum < 0.0000000000001;
TMotRP(toDelete, :) = [];
TMotRP.Sum = [];
TMotRP.Subject = char(TMotRP.Subject);
TMotRP.ID = TMotRP.Subject(:,6:8);
TMotRP_Final = sortrows(TMotRP,'ID');
TMotRP_Final.ID = [];
TMotRP_Final.Subject =[];

TMotTP.Sum = (TMotTP.T_PAUD + TMotTP.T_PCON + TMotTP.T_PMOT + TMotTP.T_PTAC + TMotTP.T_PVIS);
toDelete = TMotTP.Sum < 0.0000000000001;
TMotTP(toDelete, :) = [];
TMotTP.Sum = [];
TMotTP.Subject = char(TMotTP.Subject);
TMotTP.ID = TMotTP.Subject(:,6:8);
TMotTP_Final = sortrows(TMotTP,'ID');
TMotTP_Final.ID = [];
TMotTP_Final.Subject =[];

%Now that we have 5 small tables cleaned up to ONLY contain rows where
%there is some outlier, these small tables can be "displayed", for later
%publsihing into a pdf. 

%The sections in Purple are essentially headings. disp('Behavioral Flags')
%means font with that text will appear on the pdf.  The disp('         ')
%just adds a space for aesethic.  Next, dip(TBehav_Final) displays the
%table of behavioral outliers.  This is repeated for all the small tables
%we made.  Thus, when this script is called by the other script, flags,
%what is below is printed onto a pdf and saved. 

disp('Behavioral Flags')
disp('                                                                                          ')
disp(TBehav_Final)
disp('Rotation Placebo Flags')
disp('                                                                                          ')
disp(TMotRP_Final)
disp('Rotation Drug Flags')
disp('                                                                                          ')
disp(TMotRD_Final)
disp('Translation Placebo Flags')
disp('                                                                                          ')
disp(TMotTP_Final)
disp('Translation Drug Flags')
disp('                                                                                          ')
disp(TMotTD_Final)
